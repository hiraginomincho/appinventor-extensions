...
class SDK26Helper {
  ...
  static void askForPermission(final Sentiment sentiment, final Runnable next) {
    sentiment.getForm().askPermission(Manifest.permission.CAMERA, new PermissionResultHandler() {
      @Override
      public void HandlePermissionResponse(String permission, boolean granted) {
        if (granted) {
          next.run();
        } else {
          sentiment.getForm().PermissionDenied(sentiment, "WebViewer", permission);
        }
      }
    });
  }
