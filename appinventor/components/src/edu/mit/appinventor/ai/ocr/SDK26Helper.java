...
class SDK26Helper {
  ...
  static void askForPermission(final OCR ocr, final Runnable next) {
    ocr.getForm().askPermission(Manifest.permission.CAMERA, new PermissionResultHandler() {
      @Override
      public void HandlePermissionResponse(String permission, boolean granted) {
        if (granted) {
          next.run();
        } else {
          ocr.getForm().PermissionDenied(ocr, "WebViewer", permission);
        }
      }
    });
  }
