...
class SDK26Helper {
  ...
  static void askForPermission(final COCOSSD cocossd, final Runnable next) {
    cocossd.getForm().askPermission(Manifest.permission.CAMERA, new PermissionResultHandler() {
      @Override
      public void HandlePermissionResponse(String permission, boolean granted) {
        if (granted) {
          next.run();
        } else {
          cocossd.getForm().PermissionDenied(cocossd, "WebViewer", permission);
        }
      }
    });
  }
